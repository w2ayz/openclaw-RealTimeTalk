...
- **`RealTimeTalk-config-lib.sh` (RTT-Config / installer) now accepts
  ...
  rather than a hard rejection. `Deployment.md` updated to match. **Ported
  to the Mac fork in its own v3.25.2** (adapted: Mac's `ensure_provider_key`
  takes the prefix regex as a parameter rather than switching on `$prov`
  internally, so the port added an optional `<hint>` parameter for the
  human-readable "expected X..." wording instead of deriving it from the
  regex).
- **The daemon ignored SIGTERM (and `/stop`) while auto-sleeping**, so
  ...
  `_request_stop()` that also sets `_wake_event`. **Not applicable to the
  Mac fork** — confirmed its equivalent sleep-wait already races
  `wake_task`/`stop_task` via `asyncio.wait(..., FIRST_COMPLETED)` rather
  than blocking on a thread-pool executor, so its signal handler
  (`stop_event.set()`) already interrupts the wait immediately; it was
...
### Changed
- **STT engine settings moved out of `openclaw.json` into the daemon's own config file: `~/.openclaw/workspace/rtt_stt_config.json`** (same shape the old `talk.stt` block used). OpenClaw's TalkSchema has no `stt` key, so keeping the block in `openclaw.json` caused a recurring mess: the gateway **stripped it on every config rewrite** (silent STT revert on next daemon restart), **blocked config hot-reloads** while it was present, and `openclaw models status` / `config validate` **hard-failed** with "Unrecognized key: stt". Resolution priority unchanged: CLI `--stt-engine` > `rtt_stt_config.json` > legacy `openclaw.json` `talk.stt` (still read for unmigrated configs) > key-availability auto > openai. Same fix as the Mac fork v3.22.4.
...
- **Mic↔radio AGC mode switches stopped actually reaching speech recognition, even though logs claimed success — a USB mic kept feeding STT/wake-word detection while the AIOC radio was supposedly active.** Root cause was two-layered. First, RTT routes all mic audio through a single virtual PipeWire source (`rtt_agc_source`) that gets re-pointed at either the radio or a fallback mic on mode switch, but the already-running wake-word listener and main STT capture streams (opened once via the ALSA "pipewire" device) never re-bind to a changed default source on their own — they just keep reading from whatever concrete node was live when they first connected. Second, and the actual trigger: `_apply_agc_profile()`/`_update_agc_capture_source()` also write a PipeWire config file (`~/.config/pipewire/pipewire.conf.d/99-rtt-agc.conf`) that PipeWire itself auto-loads an echo-cancel module from at its own startup, for resilience across a PipeWire restart — but every hot-swap's attempt to unload *that* module via `pactl unload-module` silently returned "Access denied" (live-confirmed 2026-08-16: it's a native `context.modules` entry, not a pulse-compat module, so only `pw-cli destroy` can remove it). Left alive, it produced a permanent, stale duplicate node under the identical name `rtt_agc_source` — and PipeWire/WirePlumber's default-source resolution was observed binding the daemon's real capture client to that *stale* node regardless of what the live-managed one was pointed at. Fixed by (1) falling back to `pw-cli destroy` when `pactl unload-module` fails on the config-loaded module, eliminating the duplicate-node collision at its source, and (2) adding a generation counter (`_mic_restart_gen`) bumped on every mode switch that the wake-word listener and main STT capture watcher now check, explicitly closing and reopening their stream to follow it — belt-and-suspenders in case a duplicate ever recurs. Verified live by toggling mic→radio→mic three times via the dashboard: exactly one `rtt_agc_source` node existed at all times (previously it doubled), and the daemon's actual capture stream migrated to the correct new node on every switch.
